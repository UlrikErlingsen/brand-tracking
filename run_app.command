...
PID_FILE=".venv/.tracksignal.pid"
PORT_FILE=".venv/.tracksignal.port"
...
if [ -f "$PID_FILE" ] && [ -f "$PORT_FILE" ]; then
  ...
  if /bin/kill -0 "$EXISTING_PID" 2>/dev/null && /usr/bin/curl -fsS "${EXISTING_URL}/_stcore/health" >/dev/null 2>&1; then
    echo "TrackSignal is already running. Opening it now."
    if [ "${TRACKSIGNAL_NO_BROWSER:-0}" != "1" ]; then
...
READY_FILE=".venv/.tracksignal-requirements-${REQUIREMENTS_HASH}"
if [ ! -f "$READY_FILE" ]; then
  echo "First launch: downloading TrackSignal's packages. Later launches will be faster."
  python -m pip --disable-pip-version-check install --prefer-binary -r requirements.txt
  /bin/rm -f .venv/.tracksignal-requirements-* .venv/.tracksignal-ready
  /usr/bin/touch "$READY_FILE"
else
  echo "Using the existing TrackSignal environment."
fi

if [ -n "${TRACKSIGNAL_PORT:-}" ]; then
  PORT="$TRACKSIGNAL_PORT"
else
  PORT="$(python - <<'PY'
import socket

for candidate in [8586, *range(8501, 8600)]:
    sock = socket.socket()
    try:
...
echo "Starting TrackSignal at ${URL}..."
...
while [ "$ATTEMPT" -le 120 ]; do
  if /usr/bin/curl -fsS "${URL}/_stcore/health" >/dev/null 2>&1; then
    echo "TrackSignal is ready. Opening your browser..."
    if [ "${TRACKSIGNAL_NO_BROWSER:-0}" != "1" ]; then
      /usr/bin/open "$URL"
    fi
    wait "$APP_PID"
  ...
  if ! /bin/kill -0 "$APP_PID" 2>/dev/null; then
    echo "TrackSignal stopped before it became ready. Review the message above."
    wait "$APP_PID"
    exit $?
  fi
  ATTEMPT=$((ATTEMPT + 1))
...
echo "TrackSignal took too long to start. Review the message above, then try again."
exit 1
